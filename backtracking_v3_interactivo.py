    ...
    for orientacion in ["H", "V"]:
        ...
        for fila in range(len(tablero)):
            ...
            if not verifica_fila_demanda(fila, barco_actual, demanda_filas, orientacion):
                ...
                # Mostrar intento
                t = mostrar_intento_temporal(tablero, fila, 0, barco_actual, orientacion)
                mostrar_tablero(t, demanda_filas, demanda_columnas)
                print(f"Barcos intentados en esta rama : {barcos_intentados}")

                continue
            for col in range(len(tablero[0])):

                # Mostrar Intento
                t = mostrar_intento_temporal(tablero, fila, col, barco_actual, orientacion)
                mostrar_tablero(t, demanda_filas, demanda_columnas)
                print(f"Barcos intentados en esta rama : {barcos_intentados}")
                print(f"Barcos disponibles : {lista_barcos}")
                ...
                if es_valido(tablero, fila, col, barco_actual, orientacion, demanda_columnas):
                    ...
                    print(f"Barcos intentados en esta rama : {barcos_intentados}")
                    print(f"Barcos disponibles : {lista_barcos}")
                    print(f"Max. fila anterior : {max_fila_ant}, y el actual : {max_fila_act}")
                    print(f"Max. col anterior : {max_col_ant}, y el actual : {max_col_act}")
                    print(f"Espacio anterior : {espacio_ant}, y el actual : {espacio_act}")
...
def comparar_estados_tableros(max_fila_ant, max_col_ant, espacio_ant, 
    ...
    if (max_fila_ant == max_fila_act and max_col_ant == max_col_act):
